...
def get_diag_rows(board):
    ...
    for diag_row_num in range(2):
        if diag_row_num == 0:
            idx = [0, 0]  # start point of downwards line
            row_shift, col_shift = 1, 1

        elif diag_row_num == 1:
            idx = [max_index, 0]  # start point of upwards line
            row_shift, col_shift = 1, 1

        # read each box in the diagonal line
        diag_row = []
        for box in range(diag_length):
            diag_row.append(board[idx[0]][idx[1]])

            # shift the index positions
            idx[0], idx[1] = \
                idx[0] + row_shift, idx[1] + col_shift
